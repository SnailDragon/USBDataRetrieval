...
fully support LFN /  specification. /  When use stack for the working buffer,
take care on stack overflow. When use heap /  memory for the working buffer,
memory management functions, ff_memalloc() and /  ff_memfree() in ffsystem.c,
...
sufficient for /  the file names to read. The maximum possible length of the
read file name depends
...
/  At the tiny configuration, size of file object (FIL) is shrunk FF_MAX_SS
...
/* The option FF_FS_NORTC switches timestamp function. If the system does not
